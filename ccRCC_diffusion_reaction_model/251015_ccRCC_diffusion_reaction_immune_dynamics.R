##Load libraries
##Model below is explored in Suppl. Materials II of our preprint.

##if forcing R to install only binary versions, uncomment below:
#options(pkgType = "binary")

##Install from CRAN with binaries only
...
##Note that the below analysis explores a hypothesis that relies on the assumption that the metastatic potential of (VHL) ccRCC renal tumours is negated by the tumour TME;
#in particular; we assume that the metastatic potential of a tumour is limited for as long as the tumour is in equilibrium with a functioning, cytotoxic immune system
...
simulate_profiles <- function(R, N = 400) {
  ...
  I_thresh <- 0.2 #Immune cell density below which tumour is considered "immune suppressed" for our score (IDS)
  ...
  ##IDS zone: when functioning immune cells cannot infiltrate accordingly/ infiltrated cells become exhausted/dysfunctional
  IDS_zone <- which(I < I_thresh)
  IDS_frac <- length(IDS_zone) / N
  
  tibble(
    x = x, L = L, I = I, O = O,
    IDS = x %in% x[IDS_zone],
    radius = R,
    IDS_fraction = IDS_frac
  )
}
...
##IDS emergence 
IDS_summary <- all_results %>%
  dplyr::group_by(radius) %>%
  dplyr::summarise(
    median_IDS = median(IDS_fraction),
    lower = quantile(IDS_fraction, 0.25),
    upper = quantile(IDS_fraction, 0.75)
  )


ggplot(IDS_summary, aes(x = radius, y = median_IDS)) +
